...
import { WomenPromptInput } from 'src/app/models/womenPromptInput.model';
...
@Component({
  ...
  imports: [
    ...
    SelectionSummaryComponent, IonRefresher, IonRefresherContent
  ],
})
export class OutfitWomenPage {
  ...
  womenCategoriesData: ResponseClosetCategory[] = [];
  activeCategory: WomenCategoryKey | null = null;
  selectedStyle: ClosetItems | null = null;
  selectedStyles: Record<WomenCategoryKey, ClosetItems | null> = {};
  selectionSummary: SelectionItem[] = [];

  private selectedItemPrompts: Record<WomenCategoryKey, string | null> = {};
  ...
  ngOnInit(): void {
    this.isLoading = true;
    this.categoryService.getAllWomenen(GroupType.Women).subscribe((categories: ResponseClosetCategory[]) => {
      console.log('Women Categories:', categories);
      this.fixImagePaths(categories, this.imageurl);
      this.womenCategoriesData = categories;

      // انتخاب اولین دسته و دریافت آیتم‌های آن
      if (categories && categories.length > 0) {
        const firstCategory = categories[0];
        this.activeCategory = firstCategory.title as WomenCategoryKey;
  ...
  toggleCategory(key: WomenCategoryKey) {
    ...
    const normalizedKey = key.toLowerCase() as WomenCategoryKey;
    ...
    const category = this.womenCategoriesData.find(cat => cat.title.toLowerCase() === normalizedKey);
  ...
  getStylesForActiveCategory(): ClosetItems[] {
    ...
    const activeCategory = this.womenCategoriesData.find(cat => cat.title.toLowerCase() === this.activeCategory!.toLowerCase());
  ...
  toggleStyle(style: ClosetItems) {
    ...
    const catKey = cat.toLowerCase() as WomenCategoryKey;
  ...
  async uploadFile() {
    ...
    try {
      ...
      // ساخت WomenPromptInput
      const womenPromptInput: WomenPromptInput = {
      ...
      console.log('📤 Sending WomenPromptInput:', womenPromptInput);

      // آپلود فایل و داده‌ها به سرور
      const res = await lastValueFrom(
        this.closetService.uploadWomenenData(this.selectedUserFile, womenPromptInput)
      );
  ...
  onRemoveSelection(item: SelectionItem) {
    ...
    const categoryKey = item.category.toLowerCase() as WomenCategoryKey;
  ...
  onClearAllSelections() {
    ...
    this.selectedStyles = Object.keys(this.selectedStyles).reduce((acc, key) => {
      acc[key as WomenCategoryKey] = null;
      return acc;
    }, {} as Record<WomenCategoryKey, ClosetItems | null>);
